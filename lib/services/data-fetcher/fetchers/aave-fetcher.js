...
class FetcherAave extends Fetcher {
   ...
   */
  async execute(user) {
    // console.log(`======================= aave-fetcher-execute ===================`)
     ...
     * Check amount
     */
    if (collateral.addresses.length === 0) return { user, del: true, hf, borrow, reason: "No collateral", assets }

    if (borrow.addresses.length === 0) return { user, del: true, hf, collateral, reason: "No borrow", assets }
    ...
    if (hf < minHfLiq || hf > maxHfLiq) {
      ...
      return {
        user,
        health_factor: hf, // TODO chech hf!
