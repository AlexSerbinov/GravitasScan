...
const { USER_SKIPPED_BY_LOW_HF, USER_SKIPPED_BY_HIGH_HF, USER_SKIPPED_BY_LOW_COLLATERAL, USER_SKIPPED_BY_LOW_BORROW, USERS_SKIPPED_BY_FILTERS, USER_ACCEPTED, SIMULATION_RESULT, SIMULATION_ERROR, PROVIDER_ERROR } = require("../../../../configs/eventTopicsConstants")
      ...
      for (const userData of usersData) {
        try {
          if (filteredUsers.length + scippedUsers.length == usersData.length - 1) {
            ...
            console.log(`1----=-----=----=----=----=----=----- scippedUserAddresses -----=-----=-----=-----=-- 1`)
            console.log(scippedUserAddresses);
            console.log(`2----=-----=----=----=----=----=----- scippedUserAddresses -----=-----=-----=-----=-- 2`)
            
            this.emit("info", `all [skipped] users by some of filiter param:  ${scippedUserAddresses.join(", ")}`, USERS_SKIPPED_BY_FILTERS)
            // TODO but it's not for addresses which
          }
          ...
          if (calcHealthFactor < min_health_factor) {
            ...
            if (calcHealthFactor > min_health_factor * 0.25) {
          ...
          if (calcHealthFactor > max_health_factor) {
            scippedUsers.push({ user })
            if (calcHealthFactor < max_health_factor * 2) {
              // Show only users with nearest values of hf to filters, to reduce the amount of logs in the logger
          ...
          if (totalCollateralETHB.lt(minCollateralAmount)) {
            scippedUsers.push({ user })
            if (totalCollateralETHB.gt(minCollateralAmount * 0.25)) {
              // Show only users with nearest values of hf to filters, to reduce the amount of logs in the logger
              this.emit("info", `-user: ${user} [skipped] | totalCollateralETH: ${totalCollateralETHB / 10 ** 18}, minCollateral: ${minCollateralAmount / 10 ** 18}`, USER_SKIPPED_BY_LOW_COLLATERAL)
            }
            continue
          }
          ...
          if (totalBorrowsETHB.lt(minBorrowAmount)) {
            scippedUsers.push({ user })
            if (totalBorrowsETHB.gt(minBorrowAmount * 0.25)) {
              // Show only users with nearest values of hf to filters, to reduce the amount of logs in the logger
              this.emit("info", `-user: ${user} [skipped] | totalBorrowsETH: ${totalBorrowsETHB / 10 ** 18}, maxBorrow: ${minBorrowAmount / 10 ** 18}`, USER_SKIPPED_BY_LOW_BORROW)
            }
            continue
          }
          filteredUsers.push({ user })
          this.emit("info", `+user: ${user}, HF: ${calcHealthFactor}, totalCollateralETH: ${totalCollateralETHB / 10 ** 18}, totalBorrowsETH: ${totalBorrowsETHB / 10 ** 18}`, USER_ACCEPTED)
        } catch (e) {
