...
const {
  ...
  NO_USERS_TO_FIND_BORROW_AND_COLLATERAL,
  USER_ACCEPTED_BY_HF_AFTER_SIM_RESULT,
...
class TransmitFetcher extends EventEmitter {
  ...
  constructor(filters, maxNumberOfUsersToSimulation, config, simulator) {
    ...
    this.maxNumberOfUsersToSimulation = maxNumberOfUsersToSimulation
  }
  ...
  async getUsersByAsset(assets) {
    try {
      ...
      for (let i = 0; i < assets.length; i++) {
        ...
        this.emit("info", `Users by asset: ${asset} | ${userFromReddis.length !== 0 ? `number of users: ${userFromReddis.length}, users: ${userFromReddis}` : "no users from redis by given asset"}`, USERS_FROM_REDIS_TO_SIMULATION_BY_ASSEST)
      }
      ...
      console.log(`all unique users count: ${allUsers.length} by all assets`)
    ...
    try {
      ...
      simulationResult.forEach((result, index) => {
        ...
        */
        ...
        if (result.success) {
          this.emit("info", `simulation for ${userToSimulate.length} users passed with success status`, SIMULATION_PASSED_WITH_STATUS_SUCCESS)
          simulateData.push(result)
        } else {
          this.emit("info", `simulation for ${userToSimulate.length} users failed with error status`, SIMULATION_PASSED_WITH_STATUS_FAILED)
        }
      })
   ...
   * @note decodeData example:
      totalCollateralBase: BigNumber { _hex: '0x1b552bf8a75e', _isBigNumber: true },
      totalDebtBase: BigNumber { _hex: '0x14ec048477e4', _isBigNumber: true },
      availableBorrowsBase: BigNumber { _hex: '0x88c31eb993', _isBigNumber: true },
      currentLiquidationThreshold: BigNumber { _hex: '0x1fa4', _isBigNumber: true },
      ltv: BigNumber { _hex: '0x1eaa', _isBigNumber: true },
      healthFactor: BigNumber { _hex: '0x0eaf76698768577a', _isBigNumber: true }
      CanLiquidate: (only for Compound)
   */
  filterUserByHfBySimulationResult(data) {
    const simulateData = data.simulateData
    const usersByAsset = data.usersByAsset // {array} always contain <= maxNumberOfUsersToSimulation users
    ...
    for (let i = 0; i < simulateData.length; i++) {
      ...
      console.log(`1----=-----=----=----=----=----=----- decodeData -----=-----=-----=-----=-- 1`);
      console.log(usersByAsset[i]);
  
      // Convert BigNumber to normal number
      const numbers = decodeData.map(item => item._isBigNumber ? item.toHexString() : item);
      const normalNumbers = numbers.map(hex => parseInt(hex, 16).toString());
      console.log(decodeData);
      console.log(normalNumbers);
  
      console.log(`2----=-----=----=----=----=----=----- decodeData -----=-----=-----=-----=-- 2`);
      ...
      usersToNextStageFiltering.push({ user: usersByAsset[i], hf })
      this.emit("info", `user: ${usersByAsset[i]} [accepted] by HF with simulation result | user HF: ${hf}, maxHfLiq: ${maxHfLiq}, minHf: ${minHfLiq}. This user goes to the next stage: find suitable borrow and collateral`, USER_ACCEPTED_BY_HF_AFTER_SIM_RESULT)
    }

    if (usersToNextStageFiltering.length === 0) this.emit("info", "no users to execute (find suitable borrow and collateral)", NO_USERS_TO_FIND_BORROW_AND_COLLATERAL)

    return usersToNextStageFiltering
  }

  /**
   * create request for simulate every this.maxNumberOfUsersToSimulation (usual 30 - 50) users
   */
  async request(user, rawTransmit, last) {
    try {
      this.buffer.push(user)
      if (this.buffer.length >= this.maxNumberOfUsersToSimulation || last) {
   ...
   * 1-st: usersToSimulation: from 1 to maxNumberOfUsersToSimulation.
   * If usersToSimulation > maxNumberOfUsersToSimulation, "response" called multiple times by previous function.
  ...
  async handleSimulationResponse(data) {
    ...
    let usersToNextStageFiltering = this.filterUserByHfBySimulationResult(data)
