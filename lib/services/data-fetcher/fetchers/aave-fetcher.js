...
class FetcherAave extends Fetcher {
  ...
  async execute(user) {
     ...
     * Check hf
     */
    if (hf < minHfDel) return { user, del: true, hf, reason: "low hf", assets }
    this.emit("info", `user: ${user} [skipped] |  HF: ${hf} , minHF: ${minHfDel}`, "user_liquidation_params_skipped")

    if (hf > maxHfDel) return { user, del: true, hf, reason: "High hf", assets }
    this.emit("info", `user: ${user} [skipped] |  HF: ${hf}  maxHF /${maxHfDel}`, "user_liquidation_params_skipped")
    ...
    if (collateral.addresses.length === 0) {
      this.emit("info", `user: ${user} [skipped] | no collateral`, "user_liquidation_params_skipped")
      return { user, del: true, hf, borrow, reason: "No collateral", assets }
    }
    if (borrow.addresses.length === 0) {
      this.emit("info", `user: ${user} [skipped] | no borrow`, "user_liquidation_params_skipped")
    ...
    if (hf < minHfLiq || hf > maxHfLiq) {
       ...
       */
      this.emit("info", `user: ${user} [watch] |  HF: ${hf} , minHF: ${minHfLiq} maxHF: ${maxHfLiq}`, "user_liquidation_params_watch")
