...
const getArchiveData = async (protocol, column) => {
  const redisClient = await getRedisClient()
  const result = await redisClient.hget(`archive:${protocol}`, column) //TODO @AlexSerbinov check if massive arrays works fine
  return result
}
...
const setArchiveData = async (protocol, column, data) => {
  const redisClient = await getRedisClient()
  const result = await redisClient.hset(`archive:${protocol}`, column, JSON.stringify(data))
  return result
}
...
const getLastModifyKey = async protocol => {
  const redisClient = await getRedisClient()
  const result = await redisClient.get(`last_modify:${protocol}`)
  return result
}
...
const setLastModifyKey = async (protocol, date) => {
  const redisClient = await getRedisClient()
  const result = await redisClient.set(`last_modify:${protocol}`, date)
  return result
}
