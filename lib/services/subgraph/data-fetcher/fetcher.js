...
// import events logger constants from eventTopicsConstants
const { USER_SKIPPED_BY_LOW_HF, USER_SKIPPED_BY_HIGH_HF, USER_SKIPPED_BY_LOW_COLLATERAL, USER_SKIPPED_BY_LOW_BORROW, USER_ACCEPTED, SIMULATION_RESULT, SIMULATION_ERROR, PROVIDER_ERROR, USERS_SKIPPED_BY_TO_DIFFERENT_HF, USERS_SKIPPED_BY_FILTERS } = require("../../../../configs/eventTopicsConstants")
...
class Fetcher extends EventEmitter {
  ...
  async loadUsersReserves({ addresses, blockTag }) {
    try {
      ...
      for (const userData of usersData) {
        try {
          ...
          if (calcHealthFactor < min_health_factor) {
            scippedUsers.push({ user, hf: calcHealthFactor.toString() })
            // if (calcHealthFactor > min_health_factor * 0.1) {
            // scippedUsers = scippedUsers.filter(item => item.user !== user)
          ...
          if (calcHealthFactor > max_health_factor) {
            scippedUsers.push({ user, hf: calcHealthFactor.toString() })
            // if (calcHealthFactor < max_health_factor * 5) {
            // scippedUsers = scippedUsers.filter(item => item.user !== user)
          ...
          if (totalCollateralETHB.lt(minCollateralAmount)) {
            scippedUsers.push({ user, hf: calcHealthFactor.toString(), totalCollateralETH: totalCollateralETHB.dividedBy(10 ** 18).toString() })
          ...
          if (totalBorrowsETHB.lt(minBorrowAmount)) {
            scippedUsers.push({ user, hf: calcHealthFactor.toString(), totalCollateralETH: totalCollateralETHB.dividedBy(10 ** 18).toString(), totalBorrowsETH: totalBorrowsETHB.dividedBy(10 ** 18).toString() })
          ...
          this.emit("info", `+user: ${user}, HF: ${calcHealthFactor}, totalCollateralETH: ${totalCollateralETHB.dividedBy(10 ** 18).toString()}, totalBorrowsETH: ${totalBorrowsETHB.dividedBy(10 ** 18).toString()}`, USER_ACCEPTED)
        } catch (e) {
      ...
      // if (scippedUsers.length > 0) {
      // this.emit("info", `users [skipped] by to differnt HF:  ${JSON.stringify(scippedUsers)}`, USERS_SKIPPED_BY_TO_DIFFERENT_HF)
      // this.emit("info", `users [skipped] by to differnt HF:  ${scippedUsers}`, USERS_SKIPPED_BY_TO_DIFFERENT_HF)
      this.emit("info", scippedUsers, USERS_SKIPPED_BY_FILTERS)
      // }
