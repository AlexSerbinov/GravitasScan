      ...
      for (const userData of usersData) {
        try {
          ...
          if (calcHealthFactor < min_health_factor || calcHealthFactor > max_health_factor) {
            this.emit("info", `user: ${user} [skipped] |  HF: ${calcHealthFactor} , minHF: ${min_health_factor} maxHF /${max_health_factor}`, "user_liquidation_params_skipped")
            continue
          }
          ...
          if (totalBorrowsETHB.lt(minBorrowAmount)) {
            this.emit("info", `user: ${user} [skipped] | totalBorrowsETH: ${totalBorrowsETHB / 10 ** 18}, minBorrow: ${minBorrowAmount / 10 ** 18}`, "user_liquidation_params_skipped")
            continue
          }
