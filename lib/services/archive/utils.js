...
/**
 * Get the count of users in the archive for a given protocol.
 * @param {string} protocol - The protocol for which to get the users count.
 * @returns {Promise<number|boolean>} - The count of users in the archive, or false if there are no users.
 */
const getUsersCount = async protocol => {
  try {
    // Fetch the archive data for both 'archive_users'
    const archiveUsersData = await getArchiveData(protocol, "archive_users")
    let allUsers
