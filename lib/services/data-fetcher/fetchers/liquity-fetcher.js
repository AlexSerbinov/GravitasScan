...
class FetcherLiquity extends Fetcher {
  ...
  async execute(user) {
    ...
    if (!debt.gt(zero))
    return { user, del: true, coll, reason: 'No borrow'}

    if (!coll.gt(zero))
    return { user, del: true, debt, reason: 'No collateral'}
    ...
    if (hf < minHfDel)
    return { user, del: true, reason: 'low hf' }

    if (hf > maxHfDel)
    return { user, del: true, reason: 'High hf' }
  ...
  getContract(provider) {
    return new ethers.Contract(
      PROTOCOLS_CONFIG.Liquity.TROVE_MANAGER,
      TroveManagerABI,
      provider
    )
  }
