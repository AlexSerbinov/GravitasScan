...
class TransmitFetcher extends EventEmitter {
  ...
  constructor(filters, maxNumberOfUsersToSimulate, config, simulator) {
    ...
    this.maxNumberOfUsersToSimulate = maxNumberOfUsersToSimulate
  }
      ...
      const usersFromRedis = await Promise.all(
        assets.map(async asset => {
          try {
            ...
            this.emit("info", `Users by asset: ${asset} | ${usersFromRedis.length !== 0 ? `number of users: ${usersFromRedis.length}, users: ${usersFromRedis}` : "no users from redis by given asset"}`, USERS_FROM_REDIS_TO_SIMULATION_BY_ASSEST)
   ...
   * @note {array} usersByAsset always contain <= maxNumberOfUsersToSimulate users
   */
