...
const getLatestRedisBlock = async protocol => {
  ...
  const listenerUsersData = await getArchiveData(protocol, "listener_users") //TODO @AlexSerbinov - remove
  ...
  const listenerUsers = listenerUsersData ? Object.keys(listenerUsersData) : [] //TODO @AlexSerbinov - remove

  // Combine the block numbers from both categories
  const allBlocks = [...archiveUsers, ...listenerUsers] //TODO @AlexSerbinov - fix
