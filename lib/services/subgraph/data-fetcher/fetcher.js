      ...
      for (const userData of usersData) {
        try {
          ...
          if (filteredUsers.length + scippedUsers.length == usersData.length - 1) {
            ...
            this.emit("info", `users was skipped, because filtered params differ by more than 200% from filters: ${scippedUserAddresses.join(", ")}`, "users_skipped_by_filters")
          }
          ...
          if (calcHealthFactor < min_health_factor) {
            ...
            if (calcHealthFactor > min_health_factor * 0.25) {
              // This if only for reduce logs purpose
              this.emit("info", `user: ${user} [skipped] | HF: ${calcHealthFactor} , minHF: ${min_health_factor}`, "user_skipped_by_to_low_hf")
            }
            continue
          }
          if (calcHealthFactor > max_health_factor) {
            ...
            if (calcHealthFactor < max_health_factor * 2) {
              // This if only for reduce logs purpose
              this.emit("info", `user: ${user} [skipped] | HF: ${calcHealthFactor} , maxHF: ${max_health_factor}`, "user_skipped_by_to_high_hf")
            }
            continue
          }
