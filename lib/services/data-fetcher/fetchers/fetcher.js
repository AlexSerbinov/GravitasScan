 ...
 */
const { SIMULATION_SUCCESS, SIMULATION_ERROR, USER_SKIPPED } = require("../../../../configs/loggerTopicsConstants")
...
class Fetcher extends EventEmitter {
  ...
  async fetchData({ user }) {
    ...
    if (!user || typeof user !== "string") {
      this.emit("errorMessage", `User: ${user} [skipped] it is not a valid address.`, USER_SKIPPED)
      return
    }
  ...
  getABIDecode() {
    return ["tuple(tuple(address user,uint256 totalCollateralETH,uint256 totalBorrowsETH,uint256 healthFactor) userData,tuple(address asset,uint256 collateralBalance,uint256 borrowBalance)[] reserves)[]"]
  }
}
