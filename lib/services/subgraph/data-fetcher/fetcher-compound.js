...
class FetcherCompound extends Fetcher {
  ...
  async prepareTransaction(addresses) {
    let txn = {
      chainId: 1,
    ...
    const resp = await axios.post(`http://10.10.13.7:7777/api/v1/simulate-bundle`, bundle)
  ...
  async getUsersData(addresses, blockTag) {
    const resp = await this.prepareTransaction(addresses)
