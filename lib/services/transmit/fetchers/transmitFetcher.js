...
class TransmitFetcher extends EventEmitter {
   ...
   * This data recieved directly from AAVE V3 lending pool. Not from our custom aggregator
   * Because Ostap says that it's more quicker. And spped in transmit liquidations it's very important
  ...
  filterUserByHfBySimulationResult(data) {
    ...
    for (let i = 0; i < simulateData.length; i++) {
      ...
      if (hf < minHfLiq) {
        // Show only users with nearest values of hf to filters, to reduce the amount of logs in the logger
        // if (hf > minHfLiq * 0.25) {
      ...
      usersToNextStageFiltering.push({ user: usersByAsset[i], hf })
      /* decided to add some codes to easy looking logs in log server
        + mean positive result (in this case user accepted by HF after simulation result)
        A+ mean accepted 
      */
    ...
    if (usersToNextStageFiltering.length === 0) this.emit("info", "no users to execute (find suitable borrow and collateral)", ALL_USERS_SKIPPED_BY_HF)
