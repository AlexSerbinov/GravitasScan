  ...
  _execute() {
    ...
    setImmediate(async () => {
      ...
      const timeout = setTimeout(() => {
        broke = true
        console.log(`Queue execution timeout expired ${this.execTimeout}`)
        this.emit("errorMessage", `Queue execution timeout expired ${this.execTimeout}`, "event_name_event")
      ...
      try {
        console.log(`DEBUGGING: looking for an subgraph error: Queue execution runned. Note: if falled by timeout look at this line`)
