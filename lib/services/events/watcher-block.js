  ...
  start(mode = "ws", interval = 10) {
    ...
    if (this.mode === "ws" || this.mode === "wss") {
      this.provider.on("block", number => {
        ...
        console.log(`new block ${number} received via ${this.mode}, ${date}`)
        this.events.emit("block", number)
        this.resetProviderAndReattachListener() // TODO: think with @Serhiy Didkovskiy
        ...
        // Also posible to reset provider by interval (commened function resetProviderPeriodically() ), bit it also crutch
      })
