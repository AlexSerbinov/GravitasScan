...
class TransmitFetcher extends EventEmitter {
  ...
  async executeUser(user, hf, rawTransmit) {
    ...
    if (liq) {
      ...
      this.emit("info", { info: `user ${user} catched, but no need to liquidate` }, "not_liquidate_event")
    }
  }
  ...
  userToExecute(data) {
    ...
    console.log(`${userToLiquidate.length == 0 ? `no users to liquidate` : `users to Liquidate ${JSON.stringify(userToLiquidate)}`}`)

    this.emit("info", {
      info: `${userToLiquidate.length == 0 ? `no users to liquidate` : `users to Liquidate ${JSON.stringify(userToLiquidate)}`}`,
    })
