  ...
  async execute(user) {
    ...
    if (hf < minHfDel) {
      if (hf > minHfDel * 0.5) {
        ...
        this.emit("info", `user: ${user} [ del: true] |  HF: ${hf} , minHF: ${minHfDel}`, "user_liquidation_params_skipped")
      }
    ...
    if (hf > maxHfDel) {
      if (hf < maxHfDel * 1.5) {
        // only for reduce loggs purpose
        this.emit("info", `user: ${user} [ del: true] |  HF: ${hf}  maxHF /${maxHfDel}`, "user_liquidation_params_skipped")
      }
