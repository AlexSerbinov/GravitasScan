...
class Fetcher extends EventEmitter {
  ...
  async getUsersData(addresses, blockTag, blockTimestamp) {
    ...
    if (resp[0].user) {
      const message = `Simulation SUCCESS: usersData success received from simulator`
      console.log(message)
      this.emit("info", message)
    } else {
      const message = `Simulation FALLED: usersData failed to receive from simulator`
      console.log(message)
      this.emit("info", message)
    }
    return resp
  ...
  async simulateTransaction(transaction, blockTag, blockTimestamp) {
    ...
    await this.simulator.simulate(bundle, blockTag, blockTimestamp)
  ...
  async formatSimulationResult(simulationResult) {
    try {
      ...
      const message = `\nERROR: No received data from simulator`
