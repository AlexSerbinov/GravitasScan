...
class TransmitFetcher extends EventEmitter {
  ...
  async getUsersByAsset(assets) {
    ...
    for (let i = 0; i < assets.length; i++) {
      ...
      // TODO: look deeply in the logs. If user showed in logs (user_skipped_by_hf), than maube no need to send users from redis to logs
    }

    const allUsers = [...new Set([].concat(...usersFromRedis))]

    // this.emit("info", `${allUsers.length !== 0 ? `users by asset from Redis ${allUsers}` : `users from redis not exist`}`, "users_from_redis_by_asset")
    return allUsers
  }
  ...
  async simulate(userToSimulate, rawTransmit) {
    try {
      ...
      simulationResult.forEach((result, id) => {
        if (result.success) {
          this.emit("info", `simulation passed with success status`, "simulation_passed_with_success_status")
          simulateData.push(result)
        } else {
          this.emit("info", `simulation failed with error status`, "simulation_passed_with_success_status")
        }
      })

      return { simulateData, usersByAsset: userToSimulate, rawTransmit }
    } catch (error) {
      this.emit("errorMessage", error.message || error, "simulate_error")
      throw error
    }
  }
  ...
  userToExecute(data) {
    ...
    for (let i = 0; i < simulateData.length; i++) {
      ...
      if (hf < minHfLiq) {
        if (hf > minHfLiq * 0.5) {
          this.emit("info", `user: ${usersByAsset[i]} [skipped], HF: ${hf}, minHf: ${minHfLiq}`, "user_skipped_by_hf")
        }
        continue
      }
      if (hf > maxHfLiq) {
        if (hf < maxHfLiq * 1.5) {
          this.emit("info", `user: ${usersByAsset[i]} [skipped], HF: ${hf}, maxHfLiq: ${maxHfLiq}`, "user_skipped_by_hf")
        }
        continue
      }
