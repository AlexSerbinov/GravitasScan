      ...
      for (const userData of usersData) {
        try {
          ...
          if (filteredUsers.length + scippedUsers.length == usersData.length - 1) {
            ...
            this.emit("info", `users was skipped, because params are different: ${scippedUserAddresses.join(", ")}`, "user_liquidation_params_skipped")
          }
          ...
          if (calcHealthFactor < min_health_factor) {
            ...
            if (calcHealthFactor > min_health_factor * 0.7) {
              // This if only for reduce logs purpose
              this.emit("info", `user: ${user} [skipped] | HF: ${calcHealthFactor} , minHF: ${min_health_factor}`, "user_liquidation_params_skipped")
            }
            continue
          }
          if (calcHealthFactor > max_health_factor) {
            ...
            if (calcHealthFactor < max_health_factor * 1.3) {
              // This if only for reduce logs purpose
              this.emit("info", `user: ${user} [skipped] | HF: ${calcHealthFactor} , maxHF: ${max_health_factor}`, "user_liquidation_params_skipped")
            }
            continue
          }
          ...
          if (totalCollateralETHB.lt(minCollateralAmount)) {
            ...
            if (totalCollateralETHB.gt(minCollateralAmount * 0.5)) {
              // This if only for reduce logs purpose
              this.emit("info", `user: ${user} [skipped] | totalCollateralETH: ${totalCollateralETHB / 10 ** 18}, minCollateral: ${minCollateralAmount / 10 ** 18}`, "user_liquidation_params_skipped")
            }
            continue
          }
          ...
          if (totalBorrowsETHB.lt(minBorrowAmount)) {
            ...
            if (totalBorrowsETHB.gt(minBorrowAmount * 0.5)) {
              // This if only for reduce logs purpose
              this.emit("info", `user: ${user} [skipped] | totalBorrowsETH: ${totalBorrowsETHB / 10 ** 18}, maxBorrow: ${minBorrowAmount / 10 ** 18}`, "user_liquidation_params_skipped")
            }
            continue
          }
