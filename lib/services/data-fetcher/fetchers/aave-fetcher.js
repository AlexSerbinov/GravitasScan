...
const { USER_SKIPPED_BY_LOW_HF, USER_SKIPPED_BY_HIGH_HF, USER_SKIPPED_BY_HF, USER_SKIPPED_BY_LOW_COLLATERAL, USER_SKIPPED_BY_LOW_BORROW } = require("../../../../configs/eventTopicsConstants")
...
class FetcherAave extends Fetcher {
  ...
  async filterUserByLiquidationFilters(user) {
    ...
    let reserves
    let data
    if (this.params.mode === "node") {
      console.log(`this.params.mode: ${this.params.mode}`)
      data = await contract.getUserReserves(user)
      healthFactor = data.healthFactor
      reserves = data.reserves
    } else if (this.params.mode === "simulator") {
      console.log(`this.params.mode: ${this.params.mode}`)
