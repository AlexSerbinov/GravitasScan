...
class TransmitFetcherAave extends TransmitFetcher {
    ...
    async execute(user, hf) {
        ...
        if (collateral.addresses.length === 0) return $.send("info", { info: `user ${user} don't have enough collateral` })
        if (borrow.addresses.length === 0) return $.send("info", { info: `user ${user} don't have enough collateral` })
        /**
